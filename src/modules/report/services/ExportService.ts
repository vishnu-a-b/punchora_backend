...
export default class ExportService {
   ...
   */
  private exportAttendanceAnomaliesToCSV(report: any): string {
  ...
  private exportLateCheckinsToCSV(report: any): string {
    ...
    report.lateCheckIns.forEach((record: any) => {
      ...
      csv += `${record.location?.latitude || ""},${record.location?.longitude || ""}\n`;
    });
   ...
   */
  private exportAlertSummaryToCSV(report: any): string {
    // For alert summary, export the top staff with alerts
    let csv = "Staff Name,Staff UID,Alert Count\n";

    report.topStaffWithAlerts.forEach((staff: any) => {
   ...
   */
  private exportGenericToCSV(report: any): string {
   ...
   */
  private generateLocationCompliancePDF(
    ...
    const columns = [
      ...
      { header: "UID", width: 10, align: "center" as const },
      { header: "Business", width: 15 },
      { header: "Department", width: 15 },
      { header: "Has Location", width: 12, align: "center" as const },
      { header: "Last Location", width: 15 },
      { header: "Mocked GPS", width: 13, align: "center" as const },
    ];

    const rows = report.details.map((staff: any) => [
      staff.staffName,
      staff.staffUid || "",
      staff.business || "",
      staff.department || "",
   ...
   */
  private generateAttendanceAnomaliesPDF(
    _doc: PDFKit.PDFDocument,
    template: PDFTemplate,
    report: any
  ) {
    ...
    const columns = [
      ...
      { header: "UID", width: 8, align: "center" as const },
      { header: "Department", width: 15 },
      { header: "Check In", width: 12, align: "center" as const },
      { header: "Check Out", width: 12, align: "center" as const },
      { header: "Anomalies", width: 23 },
    ];

    const rows = report.anomalies.map((anomaly: any) => [
   ...
   */
  private generateLateCheckinsPDF(
    _doc: PDFKit.PDFDocument,
    template: PDFTemplate,
    report: any
  ) {
    ...
    const columns = [
      ...
      { header: "UID", width: 8, align: "center" as const },
      { header: "Department", width: 15 },
      { header: "Expected", width: 12, align: "center" as const },
      { header: "Actual", width: 12, align: "center" as const },
      { header: "Delay (min)", width: 11, align: "right" as const },
      { header: "Location", width: 10, align: "center" as const },
    ];

    const rows = report.lateCheckIns.map((record: any) => [
      ...
      record.minutesLate,
      record.location?.latitude ? "Yes" : "No",
    ]);
   ...
   */
  private generateAlertSummaryPDF(
    _doc: PDFKit.PDFDocument,
    template: PDFTemplate,
    report: any
  ) {
    ...
    const typeColumns = [
      ...
      { header: "Count", width: 30, align: "center" as const },
      { header: "Percentage", width: 30, align: "right" as const },
    ];

    const typeRows = Object.entries(report.breakdown.byType).map(
    ...
    if (
      ...
      const staffColumns = [
        ...
        { header: "UID", width: 20, align: "center" as const },
        { header: "Alert Count", width: 40, align: "right" as const },
      ];
  ...
  private generateDashboardPDF(
    ...
    if (report.alerts.recentAlerts && report.alerts.recentAlerts.length > 0) {
      ...
      const alertColumns = [
        ...
        { header: "Severity", width: 15, align: "center" as const },
        { header: "Status", width: 15, align: "center" as const },
