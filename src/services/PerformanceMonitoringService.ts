...
interface RequestMetric extends PerformanceMetric {
  method: string;
  path: string;
  statusCode: number;
  userId?: string;
...
class PerformanceMonitoringService {
  ...
  trackQuery(options: {
    ...
    if (isSlowQuery) {
      console.warn(
        `[Performance] Slow query detected: ${options.name} (${options.duration}ms)\n` +
  ...
  trackRequest(options: {
    ...
    if (options.duration > 1000) {
      console.warn(
        `[Performance] Slow request: ${options.method} ${options.path} (${options.duration}ms)`
      );
  ...
  private startMemoryMonitoring(): void {
    this.memoryMonitoringInterval = setInterval(() => {
      ...
      if (memMetric.heapUsed > 512) {
        console.warn(
          `[Performance] High memory usage: ${memMetric.heapUsed}MB heap used`
        );

        sentryService.captureMessage(
          `High memory usage detected: ${memMetric.heapUsed}MB`,
          'warning'
        );
      }
   ...
   */
  private trimMetrics(arr: any[]): void {
  ...
  getMemoryStats(): {
    ...
    const avgHeapUsed = Math.round(
      this.memorySnapshots.reduce((sum, m) => sum + m.heapUsed, 0) /
      this.memorySnapshots.length
    );

    const avgHeapTotal = Math.round(
      this.memorySnapshots.reduce((sum, m) => sum + m.heapTotal, 0) /
      this.memorySnapshots.length
    );

    const avgRss = Math.round(
      this.memorySnapshots.reduce((sum, m) => sum + m.rss, 0) /
      this.memorySnapshots.length
    );
...
export { PerformanceMetric, QueryMetric, RequestMetric, MemoryMetric };
