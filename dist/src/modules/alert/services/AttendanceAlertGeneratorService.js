...
class AttendanceAlertGeneratorService {
    constructor() {
     ...
     */
    generateLateCheckinAlerts() {
        return __awaiter(this, arguments, void 0, function* (thresholdTime = new Date(), businessId) {
            var _a, _b;
            ...
            const allActiveStaff = yield Staff_1.Staff.find(staffQuery)
                .populate("business")
                .populate("department");
            let alertsCreated = 0;
                ...
                if (!attendance && new Date() >= thresholdTime) {
                    try {
                        yield this.alertService.createAlert({
                            type: Alert_1.AlertType.LATE_CHECKIN,
                        ...
                        console.error(`[AttendanceAlert] Failed to create late check-in alert for ${staff.name}:`, error);
                    }
                }
            }
            console.log(`[AttendanceAlert] Created ${alertsCreated} late check-in alerts`);
     ...
     */
    generateMissingCheckoutAlerts(businessId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const attendanceRecords = yield Attendance_1.Attendance.find(query).populate({
                path: "staff",
            ...
            for (const record of attendanceRecords) {
                ...
                try {
                    yield this.alertService.createAlert({
                        type: Alert_1.AlertType.MISSING_CHECKOUT,
                    ...
                    console.error(`[AttendanceAlert] Failed to create missing checkout alert for ${staff.name}:`, error);
                }
            }
            console.log(`[AttendanceAlert] Created ${alertsCreated} missing checkout alerts`);
            return alertsCreated;
        });
    }
    /**
     ...
     */
    generateAttendanceAnomalyAlert(attendanceId) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            ...
            const endOfDay = new Date(today);
            endOfDay.setHours(23, 59, 59, 999);
            const duplicateCount = yield Attendance_1.Attendance.countDocuments({
                staff: staff._id,
            ...
            if (anomalies.length > 0) {
                yield this.alertService.createAlert({
                    type: Alert_1.AlertType.ATTENDANCE_ANOMALY,
                    severity: Alert_1.AlertSeverity.MEDIUM,
                    staff: staff._id.toString(),
                    business: staff.business._id.toString(),
                    department: (_b = (_a = staff.department) === null || _a === void 0 ? void 0 : _a._id) === null || _b === void 0 ? void 0 : _b.toString(),
                ...
                console.log(`[AttendanceAlert] Attendance anomaly alert created for ${staff.name}`);
            }
        });
    }
    /**
     ...
     */
    checkWeekendAttendance(attendanceId) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            const attendance = yield Attendance_1.Attendance.findById(attendanceId).populate({
                path: "staff",
            ...
            if (dayOfWeek === 0 || dayOfWeek === 6) {
                ...
                console.log(`[AttendanceAlert] Weekend attendance alert created for ${staff.name}`);
            }
        });
    }
    /**
     ...
     */
    generateDailyAttendanceAlerts(businessId) {
        return __awaiter(this, void 0, void 0, function* () {
            // Generate late check-in alerts (threshold: 9:30 AM)
            const thresholdTime = new Date();
            thresholdTime.setHours(9, 30, 0, 0);
            const lateCheckin = yield this.generateLateCheckinAlerts(thresholdTime, businessId);
            // Generate missing checkout alerts (should be run in evening/night)
            const missingCheckout = yield this.generateMissingCheckoutAlerts(businessId);
            return {
                lateCheckin,
                missingCheckout,
            };
...
exports.default = AttendanceAlertGeneratorService;
